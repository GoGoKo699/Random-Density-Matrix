...
def eta(gamma,alpha,num):
    data=[]
    for i in range(num):
        rho=W(gamma,alpha,'real')/(2*np.sqrt(alpha))
...
ax1.plot(xm,ym,'b-',label=''r'$\lambda_0$')
ax1.plot(xp,yp,'b-')
ax1.plot(xlin,ylin,'m-',label=''r'$\frac{\alpha\gamma}{\sqrt{\alpha}}$')
ax1.set_xlabel(''r'$\gamma$')
ax1.set_ylabel('eigenvalues')
ax1.set_title('Global')

ax2.plot(xmbis,ymbis,'b-')
ax2.plot(xpbis,ypbis,'b-')
ax2.plot(xlinbis,ylinbis,'m-')
