...
def c_U(entropy,c, x, b, a, N, ancilla, n):
    ...
    yield gates.CallbackGate(entropy)
...
def quantum_order_finding_full(entropy, N, a):
    ...
    for i in range(len(q_reg)):
        ...
        exp = (exp**2) % N
        #a**(2**i)
        circuit.add((c_U(q_reg[i], x, b, exponents[i], N, ancilla, n)))
    circuit.add(gates.CallbackGate(entropy))
...
def quantum_order_finding_semiclassical(entropy, N, a):
    ...
    circuit.add(gates.X(x[len(x) - 1]))
    # a_i = (a**(2**(2*n - 1)))
    circuit.add(c_U(entropy, q_reg, x, b, exponents[-1], N, ancilla, n))
    circuit.add(gates.H(q_reg))
    ...
    for i in range(1, 2 * n):
        ...
        circuit.add(gates.H(q_reg))
        # a_i = (a**(2**(2*n - 1 - i)))
        circuit.add(c_U(entropy,q_reg, x, b, exponents[-1 - i], N, ancilla, n))
        angle = 0
