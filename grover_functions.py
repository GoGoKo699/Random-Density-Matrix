...
def grover_step(entropy, q, c, circuit, A, B, C, D, x, ancilla, h, rot):
...
def grover(entropy, q, constant_1, constant_2, rot, h, grover_it, nshots=100):
    ...
    for i in range(grover_it):
        circuit = grover_step(entropy, q, c, circuit, A, B, C, D, x, ancilla, h, rot)
    circuit.add(gates.M(*(A + B), register_name="preimages"))
    result = circuit(nshots=nshots)
    return result.frequencies(binary=True)


def grover_unknown_M(entropy, q, constant_1, constant_2, rot, h):
    ...
    while True:
        ...
        if it != 0:
            total_iterations += it
            result = grover(entropy, q, constant_1, constant_2, rot, h, it, nshots=1)
